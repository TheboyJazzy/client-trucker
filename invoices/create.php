...
// Suggest the next invoice number (editable by the user).
$nextIdResult = $mysqli->query('SELECT MAX(id) AS max_id FROM invoices');
$nextId = (int) ($nextIdResult->fetch_assoc()['max_id'] ?? 0) + 1;
$invoiceNumber = 'INV-' . str_pad((string) $nextId, 4, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $mysqli->prepare('SELECT id FROM invoices WHERE invoice_number = ?');
    ...
    if (empty($errors)) {
        ...
        $stmt = $mysqli->prepare(
            'INSERT INTO invoices (client_id, project_id, invoice_number, issue_date, due_date, amount, status, notes)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        );
        $stmt->bind_param('iisssdss', $clientId, $projectIdValue, $invoiceNumber, $issueDate, $dueDate, $amount, $status, $notes);
        $stmt->execute();
        $newId = $stmt->insert_id;
        $stmt->close();

        set_flash('Invoice created.');
        redirect(base_url('invoices/view.php?id=' . $newId));
    }
}
...
<div class="panel">
    <form method="post" action="" novalidate>
        ...
        <div class="form-actions">
            ...
            <a href="<?= base_url('invoices/index.php') ?>" class="btn">Cancel</a>
        </div>
    </form>
</div>
